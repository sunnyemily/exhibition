...
;!function (win) {
     ...
     * @param  {[type]}  h     		[高]
     * @param  {[type]}  full     	[最大化]
     * @param  {[type]}  scrollbar  [是否允许浏览器出现滚动条]
     * @return {[type]}        		[description]
     */
    Xadmin.prototype.open = function (title, url, w, h, full, scrollbar) {
        if (title == null || title == '') {
            var title = false;
        }
        if (url == null || url == '') {
...
function loadHeight(){
	var iheight = parent.window.document.body.clientHeight;//document.body.clientHeight;	
	//if(iheight>680){
	//iheight=iheight;
	//}	
	//alert(iheight);
	$(".ibox-content").attr("style","height:"+iheight+"px");
}

function yanzheng(layer,msg){
...
function yanzhengshenfen(layer){
	$.ajax({
		url : "/manage/system/getAuthorizedFunctions",		
		dataType : "json",
		type : "get",async:false, 
		success : function(res) {
			if(res.status==5){
				layer.confirm(res.msg, {icon:3,
					  btn: ['确定','取消'] //按钮
					}, function(){
					   parent.window.location.href='/manage/login.html';
					}, function(){}
...
function SeeImage(val){
	if(
		val.indexOf('.html')>=0 ||
		val.indexOf('/images/null200x200.jpg')>=0 ||
		val.indexOf('/images/1.jpg')>=0 ||
		val.indexOf('/images/2.jpg')>=0 ||
		val.indexOf('/images/4.jpg')>=0 ||
		val.indexOf('/images/null150x85.jpg')>=0 ||
		val.indexOf('/images/null200x265.jpg')>=0 ||
		val.indexOf('/images/null250x141.jpg')>=0 
	){
		return;
	}
	window.open(val);
