...
function rejoin(obj) {
	layer.confirm(title2, function (index) {
		layer.close(index);
		...
		if (obj) {//删除单行
			...
			if (checkStatus.data.length == 0) {
				layer.msg(usernameValidateTips1, { icon: 5 });
				return;
			}
			else {
		...
		$.post("/api/carcard/rejoin", { idList: idList },
			function (data) {
				layer.close(index);
				if (data.status == 1) {
					//先更新本地数据
