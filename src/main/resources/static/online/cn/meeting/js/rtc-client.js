...
class RtcClient {
  ...
  onemuteAudioStreams(userId) {
    console.log(this.remoteStreams_)
    for (let stream of this.remoteStreams_) {
      if (stream.getUserId() == userId) {
        stream.muteAudio();
      }
    }
  }
  oneUnmuteAudioStreams(userId) {
    console.log(this.remoteStreams_)
    for (let stream of this.remoteStreams_) {
      if (stream.getUserId() == userId) {
        stream.unmuteAudio();
  ...
  handleEvents() {
    ...
    this.client_.on('stream-removed', evt => {
      ...
      if ($('#' + remoteStream.getId()).is(mainVideo)) {
            // 主屏视频 退出
        console.log('主屏视频 退出')
