...
<div class="mem-box" style="margin:10px auto">
	...
	<div class="mem-right">
		...
		<div class="mem-favpro">
			<ul>
			<#list scProducts as sc>
				<li>
					<div class="pic">
						<a target="_blank" href="productinfo-${sc.productId}.html">
							<img src="${sc.productPicture}" alt="">
						</a>
					</div>
					<div class="text">
						<a target="_blank" href="productinfo-${sc.productId}.html">${sc.productName}</a>
						<p>参考价：${sc.productPrice}/${sc.productUnit}</p>
						<div class="bot">
							<button onclick="quxiao(${sc.id})" class="qx"></button>
						</div>
					</div>
				</li>
			</#list>				
			...
			<div class="pages">
				<#list paging as page>
					<a ${(page.yema == pageIndex)?string('class="active"','')} ${(page.yema == 0)?string('','href="scproduct-'+page.yema+'.html"')}>${page.title}</a>
				</#list>
			</div>
		</div>
	</div>
